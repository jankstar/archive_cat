        ...
        if (this.document[lIndex]) {
          ...
          invoke("toMain", {
            message: JSON.stringify({
              path: "save_document",
              query: "",
              data: JSON.stringify(this.document[lIndex]),
            }),
          });
        }
      },
...
<template>
  <q-page padding>
    <div v-if="toggle == '0'">
      ...
      <!-- table if 0 - table -->
      <q-table
        style="height: calc(100vh - 165px); margin-top: 10px"
        :title="$t('Documents')"
        :rows="document"
        :columns="columns"
        row-key="index"
        :no-data-label="$t('empty')"
        separator="cell"
        :loading="loading"
        :filter="filter"
            ...
            <div v-if="getSelected('id') != ''" class="q-pa-md">
              <q-form @submit="onSubmit" @reset="onReset" class="q-gutter-md">
                <div class="q-gutter-xs row">
                  ...
                  <q-input v-model="detailData['date']" label="date"></q-input>
                  <q-field v-model="detailData['amount']" label="amount">
                ...
                <div class="q-gutter-xs row">
                  <q-input
                    v-model="detailData['sender_name']"
                    label="sender_name"
                    style="width: 49%"
                  ></q-input>
                  <q-input
                    v-model="detailData['recipient_name']"
                    label="recipient_name"
                    style="width: 49%"
                  >
                  </q-input>
                  <q-input
                    v-model="detailData['sender_addr']"
                    label="sender_addr"
                    style="width: 49%"
                  ></q-input>
                  <q-input
                    v-model="detailData['recipient_addr']"
                    label="recipient_addr"
                    style="width: 49%"
                  >
                  </q-input>
                </div>
        ...
        <q-card style="width: 50%; height: 100%">
          <q-card-section>
            <div class="text-h6">
              ...
              <q-btn
                ...
                icon-right="psychology"
                @click="onToggleProtocol()"
                flat
                color="primary"
              >
              ...
              <q-btn
                ...
                icon-right="smart_toy"
                @click="onToggleProtocol(2)"
                flat
                color="primary"
              >
          ...
          <q-card-section>
            <div v-if="toggleProtocol == '0' && detailData.pdfbase64 != ''">
              ...
              <iframe
                :src="detailData.url_blob ? detailData.url_blob : ''"
                style="height: 75vh; width: 100%"
              ></iframe>
            </div>
            <div v-if="toggleProtocol == '1'">
              <q-input
                v-model="selected[0]['protocol']"
                label="protocol"
                type="textarea"
                input-style="height: 756em;"
              ></q-input>
            </div>
